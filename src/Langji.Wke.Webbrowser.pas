...
procedure DombJsBingCallback(webView: TmbWebView; param: Pointer; es: TmbJsExecState; queryId: Int64; customMsg: Integer; const request: PAnsiChar); stdcall;
begin
...
begin
  ...
  FCookieEnabled := true;
  FpopupEnabled := true;
  FUserAgent := 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/49.0.1650.63 Safari/537.36 Langji.MiniBlink 1.1';
  FPlatform := wp_Win32;
  FLocalUrl := '';
  FLocalTitle := '';
...
procedure TWkeWebBrowser.DoWebViewJsCallBack(Sender: TObject; param: Pointer; es: TmbJsExecState; v: TmbJsValue);                                        //js返回值
begin
